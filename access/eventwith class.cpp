...
public:
	...
	void addtolist();//add to a list function. it calls on three functions in it. : info, turnintofile, and already in.
	void oneevent();// displays a event already in a a .txt file by asking the user for which event they want specifically.
	void allevents();// displays al events.
	...
	void info(); // adds to a list with six questions to the user. that info works with turnintofile and already in. the user will be inputting into sturcts.
...
private:
	summerplan *ideas;
	int totalsize;
	//actual size input of users input
	int actualsize;
...
{
	int i = actualsize;
	/*if (!alreadyin())//read whats in the file but if not..
	{
	...
	*/
	...
	actualsize=i;
	turnitintofile();//this function will turn it info in into a struct.
...
void pointer::info()
{
	...
	if (actualsize>= totalsize)
	{
		...
		for (int cycle=0; cycle < actualsize; ++cycle)// copies information to new array if it array is bigger then totalsize
			{
...
bool pointer::turnitintofile()//turns info from a list into a file
{
	int i = actualsize;
