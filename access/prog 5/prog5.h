...
struct summerplan // summer plan will have be dynamically allocated.
{
	//constructor
	summerplan();
	~summerplan();//deconstructor

	void onscreen(); // displays what has already been made to the user.
	void receive();
	void copyfunction(summerplan event);
	char *nameofplace; // This member is used to ask the place of event desired
	char *description; // this will carry the description array.
	char *location;	// all these arrays are char arrays because user could input num and letters in it. for example, Feburary 10th, 10, 28, 2017.
	char *dates;// will carry the date of event.
	char *question1;// will ask if they attended before
	char *question2; // will ask how often have they attendedf.
...
// the first three functions will carry the weight of the program. they will use the other functions as methods.
	...
	private:
		node *head; // dynamically allocated space
